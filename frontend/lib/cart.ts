...
export const normalizeCustomizationSelection = (customizations?: CustomizationSelectionDTO): CustomizationSelectionDTO | undefined => {
  if (!customizations) {
    return undefined;
  }
...
export const buildCartItemKey = (menuItemId: number, customizations?: CustomizationSelectionDTO) =>
  `${menuItemId}:${JSON.stringify(normalizeCustomizationSelection(customizations) ?? {})}`;
